...
const DoctorAppointments = ({setOpenMobileSidebar}:propType) => {
  ...
  return (
    <div className='w-full mx-auto'>
      <h1 className='text-xl md:text-2xl lg:text-3xl text-blue-900 font-semibold  bg-blue-300 w-full py-4 px-8 shadow-md rounded-md my-2 flex items-center justify-between gap-4'>My Appointments<MenuSquare size={30} className='block lg:hidden cursor-pointer' onClick={()=>setOpenMobileSidebar(prev=> !prev)}/></h1>
       ...
       {upcomingAppointments.length === 0 ? (
        <div className="mt-10 p-6 rounded-lg text-center text-gray-600 font-medium">
          ...
          upcomingAppointments.map((appointment) =>(
           ...
           <p ><span className='font-semibold'>Patient Name:</span> {appointment.patientId.name}</p>
           <p ><span className='font-semibold'>Age:</span> {appointment.patientId.age}</p>
           <p><span className='font-semibold'>Gender:</span> {appointment.patientId.gender}</p>
            <p ><span className='font-semibold'>Blood Group:</span> {appointment.patientId.bloodGroup}</p>
             <p ><span className='font-semibold'>Email:</span> {appointment.patientId.email}</p>
              <p ><span className='font-semibold'>Phone:</span> {appointment.patientId.phone}</p>
