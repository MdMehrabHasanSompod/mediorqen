"use client"
import { BriefcaseMedical, ClipboardPlus, Home, LayoutDashboard, LogOut, PlusCircle, User, Users, X } from 'lucide-react'
...
const AdminMobileSidebar = ({setCurrentMenu,setOpenMobileSidebar}:propType) => {
     ...
     <div className="flex items-center justify-between gap-4 mx-2 lg:mx-4 py-2 text-blue-900 font-semibold">
      <div className="flex items-center justify-center gap-1 cursor-pointer transition-opacity duration-600" onClick={()=>router.push("/")}>
       ...
       <hr className="text-blue-500 bg-blue-500 h-0.5" />
        ...
        <div className="flex items-center justify-start gap-1 hover:bg-blue-200 cursor-pointer text-blue-900 hover:text-blue-800  transition-opacity duration-600 px-2 lg:px-4 py-3" onClick={()=>router.push("/admin/add-doctor")}>
            <PlusCircle className='w-6 h-6'/>
            <p className="text-md lg:text-lg transition-opacity duration-300">
            Add Doctor
            </p>
          </div>  
         <div className="flex items-center justify-start gap-1 hover:bg-blue-200 cursor-pointer text-blue-900 hover:text-blue-800  transition-opacity duration-600 px-2 lg:px-4 py-3" onClick={()=>setCurrentMenu("admin-profile")}>
