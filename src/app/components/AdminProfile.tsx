...
const AdminProfile = ({setOpenMobileSidebar}:propType) => {
  const admin = useAdminStore((state)=>state.admin)
  const session = useSession()
  const setAdmin = useAdminStore((state)=>state.setAdmin)
  const [updateProfile,setUpdateProfile] = useState<boolean>(false)
  const [updatedAvatar,setUpdatedAvatar] = useState<File | null >()
  const [displayUpdatedAvatar, setDisplayUpdatedAvatar] = useState<string | undefined>(admin?.avatar as string)
  const [updatedName,setUpdatedName] = useState<string>(admin?.name as string)
  const [updatedPhone,setUpdatedPhone] = useState<string>(admin?.phone as string)
  ...
  const handleAdminUpdate = async(e:FormEvent) => {
    ...
    try {
     ...
     if (admin?._id) formData.append("id",admin._id)
     if(session.data?.user.role) formData.append("role",session.data.user.role)
     if (updatedName !== admin?.name) formData.append("updatedName", updatedName);
     if (updatedPhone !== admin?.phone) formData.append("updatedPhone", updatedPhone);
     if (updatedAvatar) formData.append("updatedAvatar", updatedAvatar);
     if (removeAvatar) formData.append("removeAvatar", "true");
      
      const result = await axios.patch("/api/admin/update-profile",formData)

      console.log(result);

      if(result.data.success){
        setAdmin(result.data.updatedAdmin)
        setUpdatedName(result.data.updatedAdmin.name);
        setUpdatedPhone(result.data.updatedAdmin.phone);
        setDisplayUpdatedAvatar(result.data.updatedAdmin.avatar);
        setUpdatedAvatar(null); 
  ...
  return (
   <div className='w-full mx-auto'>
      <h1 className='text-xl md:text-2xl lg:text-3xl text-blue-900 font-semibold  bg-blue-300 w-full py-4 px-8 shadow-md rounded-md my-2 flex items-center justify-between gap-4'>Admin Profile<MenuSquare size={30} className='block lg:hidden cursor-pointer' onClick={()=>setOpenMobileSidebar(prev=> !prev)}/></h1>
            ...
            <div className='mt-3'>
             <h3 className="text-lg font-semibold text-gray-700 mb-3">Update Avatar</h3>
                      ...
                      <div className='my-10 flex items-center justify-around gap-3'>
                        ...
                        <button onClick={handleAdminUpdate} className='bg-green-500 shadow-md px-4 py-2 rounded-md flex items-center justify-center gap-1 text-white font-semibold cursor-pointer hover:bg-green-600'>{loading && <Loader2 size={18} className='animate-spin' />}Save Update</button>
                      </div>
          </div>
       </>) :(<>
       <div className='bg-blue-200 p-6 rounded-lg flex flex-col gap-2'>
          <div className="w-36 h-36 sm:w-40 sm:h-40 mt-6 lg:w-50 lg:h-50 rounded-full overflow-hidden border-4 border-blue-400 mb-5 mx-auto">
            {admin?.avatar ? <Image className="object-cover w-full h-full" src={admin?.avatar} alt={admin?.name} width={200} height={200}/>
            :<User  className="object-cover w-full h-full bg-blue-600 text-white"/>
          }
          </div>
          <p className='text-xl text-blue-900'><span className='font-semibold'>Name:</span> {admin?.name}</p>
          <p className='text-xl text-blue-900'><span className='font-semibold'>Email:</span> {admin?.email}</p>
          <p className='text-xl text-blue-900'><span className='font-semibold'>Phone:</span> {admin?.phone === undefined ? "Not Provided":admin?.phone}</p>
          <p className='text-xl text-blue-900'><span className='font-semibold'>Joined At:</span> {admin?.createdAt ? new Date(admin.createdAt).toLocaleDateString("en-US",{
             day: "2-digit",
             month: "short",
             year: "numeric",
          }):""}</p>
      ...
      <div className='my-10'>
        ...
        <div className='flex flex-col md:flex-row items-center justify-center gap-6 mt-10'>
          ...
          <button onClick={()=>router.push("/admin/reset-password")} className='bg-blue-500 shadow-md p-3 rounded-md flex items-center justify-center gap-1 text-white font-semibold cursor-pointer hover:bg-blue-600'><RotateCcw size={18}/> Reset Password</button>
        </div>
     </div>
