...
export const PATCH = async(request:NextRequest) => {
    try {
        ...
        interface IUpdateSuperAdminData {
            name?: string;
            phone?: string;
            avatar?:string | null;
        }
        ...
        const updateData:IUpdateSuperAdminData = {};
       ...
       if (updatedAvatar) {
        ...
        const updatedSuperAdmin = await User.findByIdAndUpdate(id,updateData,{
          new:true
        })

        return NextResponse.json(
            {success:true,message:"Super Admin Updated Successfully",updatedSuperAdmin},
            {status:200}
        )
